...
# Sidebar Inputs (Extended Limits for Supercritical Support)
...
P_condenser = st.sidebar.slider("Condenser Pressure (MPa)", 0.005, 0.5, 0.1, 0.005)
...
try:
    ...
    try:
        ...
        quality = 1.0  # Captures gas phase if expanded steam lands completely outside the vapor dome
...
if valid:
    ...
    if quality is not None and quality < 0.999:
    ...
    table_data = {
        ...
        "Description": ["Saturated liquid", "Compressed liquid", "Superheated steam", "Wet/dry steam" if quality < 0.999 else "Superheated steam"],
    ...
    with col_left:
        ...
        # --- Safe Boundary Check for Line 173 ---
        if P_boiler < 22.064:
            T_sat_boiler = IAPWS97(P=P_boiler, x=0).T
        else:
            T_sat_boiler = T_boiler
        ...
        fig1.add_trace(go.Scatter(x=list(s_boiler_profile), y=list(T_boiler_profile), mode='lines', name='Rankine cycle', line=dict(color='red', width=2.5), legendgroup="cycle"))
        fig1.add_trace(go.Scatter(x=[s3, s4], y=[T_boiler, state4.T], mode='lines', line=dict(color='red', width=2.5), showlegend=False, legendgroup="cycle"))
        fig1.add_trace(go.Scatter(x=[s4, s1], y=[state4.T, state1.T], mode='lines', line=dict(color='red', width=2.5), showlegend=False, legendgroup="cycle"))
        fig1.add_trace(go.Scatter(x=[s1, s2_actual], y=[state1.T, T2_actual], mode='lines', line=dict(color='red', width=2.5), showlegend=False, legendgroup="cycle"))

        # 5. Overlay component marker dots
